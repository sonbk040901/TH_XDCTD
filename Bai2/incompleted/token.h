...
typedef enum
{
  ...
  TK_STRING, // Token một xâu ký tự vd: "Sơn đẹp trai\n"
...
typedef struct
{
  char string[MAX_STRING_LEN /*MAX_IDENT_LEN*/ + 1]; // Mảng chứa giá trị của token(Định danh, số)
  int lineNo, colNo;                                 // Vị trí của token
  TokenType tokenType;                               // Kiểu token
