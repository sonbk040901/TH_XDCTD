...
int sizeOfType(Type *type)
{
  ...
  case TP_CHAR:
    return CHAR_SIZE;
  case TP_ARRAY:
    int iterSize = sizeOfType(type->elementType);
    return (type->arraySize * iterSize);
  }
  return 0;
}
...
void declareObject(Object *obj)
{
  ...
  if (symtab->currentScope == NULL)              // Trường hợp là biến toàn cục
    addObject(&(symtab->globalObjectList), obj); //
  else
  {
    switch (obj->kind)
    {
    case OBJ_VARIABLE:                                                    // Nếu là biến
      obj->varAttrs->scope = symtab->currentScope;                        // Khởi tạo phạm vi của biến chính là phạm vi hiện tại đang thực hiện
      obj->varAttrs->localOffset = symtab->currentScope->frameSize;       // Khởi tạo địa chỉ cục bộ của biến là kích thực của khung hiện tại
      symtab->currentScope->frameSize += sizeOfType(obj->varAttrs->type); // Tăng kích thức của khung hiện tại lên phụ thuộc vào kích thước của biến
      break;
    case OBJ_FUNCTION:
    ...
    case OBJ_PROCEDURE:
      ...
      break;
    case OBJ_PARAMETER:                                                     // Nếu là tham số
      obj->paramAttrs->scope = symtab->currentScope;                        // Khởi tạo phạm vi của biến chính là phạm vi hiện tại đang thực hiện
      obj->paramAttrs->localOffset = symtab->currentScope->frameSize;       // Khởi tạo địa chỉ cục bộ của biến là kích thực của khung hiện tại
      symtab->currentScope->frameSize += sizeOfType(obj->paramAttrs->type); // Tăng kích thức của khung hiện tại lên phụ thuộc vào kích thước của biến
      Object *owner = symtab->currentScope->owner;
      switch (owner->kind)
      {
      case OBJ_FUNCTION: // Nếu là tham số của hàm
        addObject(&(owner->funcAttrs->paramList), obj);
        owner->funcAttrs->paramCount++;
        break;
      case OBJ_PROCEDURE: // Nếu là tham số của thủ tục
